...
import { getAppoval } from '../../api/owner/ownerInstance';
import Swal from 'sweetalert2';
  ...
  function OwnerLogin(){
    ...
    const OwnerLogin=async(e)=>{
       ...
       axios.post(`${ownerUrl}login`, adminData).then((response)=>{
        ...
        if (response.data.logIn) {             
          ...
          toast.error("Your Request is Denied")
          setDenied(true)
    ...
    const reApply = async() =>{
      if(email){
        const response = await getAppoval(email)
        if(response.success){
          toggleModal() 
              ...
              <div className="flex -mx-3">
                <div className="w-full px-3 mb-12">
                  ...
                  <div className="flex">
                    ...
                    <input value={password} onChange={(e)=>setPassword(e.target.value)}  type="new-password" className="w-full -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" placeholder="************" />
                  </div>
                </div>
              </div>
              <div className="flex -mx-3">
                <div className="w-full px-3 mb-5">
                  ...
                  <button onClick={()=>{navigate('/owner/ownersignup')}} className="block w-full max-w-xs mx-auto  text-blue-500 hover:text-blue-800 rounded-lg font-semibold">Register</button>
          </div>
        </div>
      </div>
    </div>
  ...
  <div className="modal">
    <div className="modal-dialog">
      ...
      <div className="modal-header text-center">
          <h2 className="modal-title mx-auto text-lg"><b>Denied</b></h2>
        </div>
        <div className="modal-body text-center">
          <img
            className="mb-3"
            src="https://uxwing.com/wp-content/themes/uxwing/download/signs-and-symbols/access-denied-icon.svg"
            style={{ width: "100px", height: "100px", margin: "0 auto" }}
            alt="Denied"
          />
          <h3></h3>
          <p style={{ fontSize: "20px", color: "#5C5C5C" }}>
            Your Request for Theatre appoval is Denied due to some reason.
            You can Reapply for the approval using the reapply option
          </p>
        </div>
        <div className="modal-footer justify-content-between">
          <a onClick={toggleModal} className="btn btn-danger text-white">
            Cancel
          </a>
          <a onClick={reApply} className="btn btn-success text-white">
            Reapply
          </a>
        </div>
      </div>
...
{block && (
  <div className="modal">
    <div className="modal-dialog">
      <div className="modal-content">
      <div className="modal-header text-center">
          <h2 className="modal-title mx-auto text-lg"><b>Blocked</b></h2>
        </div>
        <div className="modal-body text-center">
          <img
            className="mb-3"
          ...
          <h3></h3>
          <p style={{ fontSize: "20px", color: "#5C5C5C" }}>
            Your Account as been Blocked due to some Reason
          </p>
        </div>
        <div className="modal-footer justify-content-center">
          <a onClick={BlockModal} className="btn btn-info text-white">
            Ok
          </a>
...
{pending && (
  <div className="modal">
    <div className="modal-dialog">
      <div className="modal-content">
      <div className="modal-header text-center">
          <h2 className="modal-title mx-auto text-lg"><b>Pending</b></h2>
        </div>
        <div className="modal-body text-center">
          <img
            className="mb-3"
            src="https://cdn4.iconfinder.com/data/icons/coral-business/24/time-512.png "
            style={{ width: "100px", height: "100px", margin: "0 auto" }}
            alt="Denied"
          />
          <h3></h3>
          <p style={{ fontSize: "20px", color: "#5C5C5C" }}>
            Your Request for approval is pending. Please try again later.
          </p>
        </div>
        <div className="modal-footer justify-content-center">
          <a onClick={PendingModal} className="btn btn-info text-white">
            Ok
          </a>
